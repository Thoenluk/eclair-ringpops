...
public class Lsh extends ValidInstruction {
    protected Lsh(final Argument[] arguments) {
        super(arguments);
    }
    ...
    public Optional<String> checkSanityWarning() {
        ...
        if (isLiteralZero(second)) {
            return Optional.of("Shifting 0 (second argument) left by anything will always result in 0. It is recommended to use MOV <ADDRESS> 0 instead.");
        }
        ...
        if (isLiteralZero(third)) {
            ...
            return Optional.of("Shifting left by a negative offset (third argument) is possible, but effectively a right shift. It is recommended to use RSH <ADDRESS> <ARG1> <ARG2> instead with the positive value of ARG2.");
        }
